...
function Player() {
  ...
  const { currentSong, currentSongIndex,selectedPlaylist, allSongs, isPlaying, currentTime } =
    useSelector((state) => state.user);

   
    var array=allSongs;
    if(selectedPlaylist!==null){
     array=selectedPlaylist.songs;
  ...
  const onPrev = () => {
    ...
    if (currentSongIndex !== 0 && !shuffleOn) {
      ...
      dispatch(SetCurrentSong(array[currentSongIndex - 1]));
    }
    else if(currentSongIndex === 0 && !shuffleOn){
      dispatch(SetCurrentSongIndex(array.length - 1));
      dispatch(SetCurrentSong(array[array.length - 1]));
    }
    
    else {
      const randomIndex = Math.floor(Math.random() * array.length);
      dispatch(SetCurrentSongIndex(randomIndex));
      dispatch(SetCurrentSong(array[randomIndex]));
    }
  ...
  const onNext = () => {
    ...
    if (currentSongIndex !==array.length - 1 && !shuffleOn) {
      dispatch(SetCurrentSongIndex(currentSongIndex + 1));
      dispatch(SetCurrentSong(array[currentSongIndex + 1]));
    }
    else if (currentSongIndex ===array.length - 1 && !shuffleOn) {
      dispatch(SetCurrentSongIndex(0));
      dispatch(SetCurrentSong(array[0]));
    }
    else {
      const randomIndex = Math.floor(Math.random() * array.length);
      dispatch(SetCurrentSongIndex(randomIndex));
